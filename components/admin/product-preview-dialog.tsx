...
export function ProductPreviewDialog({
  ...
  const galleryImages = [...(data.images || [])].sort(
    (a, b) => a.order - b.order,
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Inspect the listing details before publishing
              </p>
            </div>
          </div>
                  ...
                  {galleryImages.length > 0 && (
                    <div className="grid grid-cols-4 gap-3">
                      {galleryImages.map((img: any, i: number) => (
                        <div
                          key={i}
                          className="aspect-square relative rounded-lg overflow-hidden border bg-white shadow-xs"
                        >
                          <Image
                            src={img.url}
                            alt="Gallery"
                ...
                <div className="md:col-span-7 space-y-8">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <Badge
                        variant="secondary"
                        className="bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border-none px-2 py-0 text-[10px] font-bold uppercase"
                      >
                        {data.categoryId || "General"}
                      </Badge>
                      {data.isFeatured && (
                        <Badge className="bg-amber-500 text-white border-none text-[10px] font-bold uppercase">
                          Featured
                        </Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-baseline gap-2">
                      <span className="text-2xl font-black text-slate-900">
                        $
                        {Number(data.titlePrice || 0).toLocaleString(
                          undefined,
                  ...
                  {data.tags && data.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 pt-2">
                      {data.tags.map((tag: string) => (
                        <div
                          key={tag}
                          className="flex items-center gap-1 px-2 py-1 rounded-md bg-white border text-[10px] font-medium text-slate-500 uppercase tracking-tighter"
                        >
